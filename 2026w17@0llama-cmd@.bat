...
set "DOCS_DIR=%AI_SOFT%\lib"
set "HISTORY_FILE=%ROOT%history.txt"
set "SYS_PROMPT=%DOCS_DIR%\syspromt.txt"

set LLAMA_FLAGS=-c 128000 ^
-ctk q8_0 -ctv q4_1 ^
--reasoning auto --reasoning-budget -1 ^
-t 6 --threads-batch 6 ^
-n -1 -ngl all --prio 3 -fa on -fit on ^
--parallel 1 --conversation --no-display-prompt --color on --jinja --keep 1

:: --system-prompt "Reasoning: none" ^ --temp 1.0 --min-p 0.1 --top-n-sigma 1.5 ^ --dry-multiplier 0.8 --dry-base 1.75 --dry-allowed-length 2 --dry-penalty-last-n 256 ^ --repeat-penalty 1.1 --repeat-last-n 128 ^ --xtc-threshold 0.1 --xtc-probability 0.5 ^ --top-k 0 --top-p 1.0 --typical 1.0 ^
...
cls
echo.
echo [+] 📟 ПОЛНЫЙ КОНФИГ ПЕРЕД ЗАПУСКОМ:
echo ---------------------------------------------------
echo ПУТЬ К CLI:  %LLAMA_CLI%
echo МОДЕЛЬ:      %SELECTED_MODEL%
echo АДАПТЕР:     !MMPROJ_ARG!
echo СИСТЕМНЫЙ:   !SYS_ARG!
echo СЕССИЯ:      !FILE_ARG!
echo ФЛАГИ:       %LLAMA_FLAGS%
echo ---------------------------------------------------
echo.

pause

"%LLAMA_CLI%" -m "%SELECTED_MODEL%" !MMPROJ_ARG! !SYS_ARG! !FILE_ARG! %LLAMA_FLAGS%
