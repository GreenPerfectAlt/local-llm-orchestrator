...
set LLAMA_FLAGS=--ctx-size 64000 -ctk q5_1 -ctv q5_1 --reasoning off --reasoning-budget 0 --mmap ^
-n -1 -ngl -1 --prio 3 -fa on -fit on --numa distribute ^
--samplers "top_k;top_p;typ_p;min_p;temperature;dry;xtc" ^
--temp 1.0 --min-p 0.1 --dynatemp-range 0.3 --xtc-threshold 0.15 --xtc-probability 0.5 --dry-multiplier 0.8 --dry-base 1.75 --dry-allowed-length 2 --dry-penalty-last-n 4096 --top-n-sigma 5.0 --presence-penalty 0.1 --frequency-penalty 0.1 --adaptive-target 0.8 --adaptive-decay 0.9 --typical 0.9 ^
--override-kv gemma.expert_used_count=int:32 --override-kv qwen3moe.expert_used_count=int:32 --override-kv gpt-oss.expert_used_count=int:32 -ctxcp 2 ^
...
"%LLAMA_SERVER%" -m "%SELECTED_MODEL%" !MMPROJ_ARG! %LLAMA_FLAGS%
