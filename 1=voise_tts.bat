...
start "" /high "%KOBOLDCPP%" --model "%SELECTED_MODEL%" !MMPROJ_ARG! --contextsize 32000 --smartcontext --threads 6 --threads-batch 6 --gpulayers -1 --highpriority --foreground --skiplauncher --usevulkan 0 --flashattention --usemmap --chat-template-kwargs "{\"enable_thinking\":false}" --websearch --smartcache 1 --jinja --ttsmodel "tts\Qwen3-TTS-12Hz-1.7B-VoiceDesign-Q8_0.gguf" --ttswavtokenizer "tts\qwen3-tts-tokenizer-q8_0.gguf" --ttsgpu --ttsthreads 6 

:: Если тебе нужно БОЛЬШЕ 8к (реальная бесконечность): Удали --useswa и оставь --smartcontext, при этом обязательно укажи --contextsize 32768 (или больше). Только в этом режиме smartcontext раскроет свой потенциал, позволяя модели помнить огромные куски текста и не тормозить при их сдвиге.
...
"%LLAMA_CLI%" ^
 ...
 --conversation --no-display-prompt --color on --jinja ^
 --reasoning off --reasoning-budget 0 ^
 -n -1 -ngl 0 -t 6 -tb 6 ^
 --cache-ram 1024 ^
 !SYS_ARG! ^
